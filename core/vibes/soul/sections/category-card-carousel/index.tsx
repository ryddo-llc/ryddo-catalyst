...
} from '../../primitives/category-card';
import * as Skeleton from '../../primitives/skeleton';
          ...
          <Carousel className={className} hideOverflow={hideOverflow}>
            <CarouselContent>
              {cards.map((card, index) => {
                const cardKey = card.href || `category-${index}`;

                return (
                  <CarouselItem
                    className="basis-full @sm:basis-1/2 @md:basis-[30%] @4xl:basis-[30%]"
                    key={cardKey}
                  >
                    <CategoryCard
                      {...card}

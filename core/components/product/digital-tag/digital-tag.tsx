...
export function DigitalTagSkeleton() {
  return (
    <div
      ...
      aria-live="polite"
      className="flex min-h-screen items-center justify-center bg-gradient-to-br from-gray-50 to-gray-100 p-2 sm:p-4 md:p-8"
      role="status"
    >
      <div className="relative w-full max-w-[320px] scale-[0.85] sm:scale-100">
        <div
          className="relative w-full animate-pulse bg-gray-300 text-white shadow-[0_2px_4px_rgba(0,0,0,0.15)] sm:shadow-[0_4px_8px_rgba(0,0,0,0.15)]"
          style={{
            clipPath:
...
function DigitalTagContent({ data }: { data: DigitalTagData }) {
  ...
  const getBrandTextClasses = (brandName: string) => {
    ...
    if (length <= 8) return 'text-3xl sm:text-4xl md:text-5xl';
    if (length <= 12) return 'text-2xl sm:text-3xl md:text-4xl';
    if (length <= 16) return 'text-xl sm:text-2xl md:text-3xl';
  ...
  const getBrandPadding = (brandName: string) => {
    return brandName.length > 16 ? 'px-3 pb-2 pt-12 sm:px-4 sm:pb-3 sm:pt-16' : 'px-4 pb-3 pt-12 sm:px-6 sm:pb-4 sm:pt-16';
  };

  return (
    <div className="flex min-h-screen items-center justify-center bg-gradient-to-br from-gray-50 to-gray-100 p-2 sm:p-4 md:p-8">
      {/* Container */}
      <div className="relative w-full max-w-[320px] scale-[0.85] sm:scale-100">
        {/* The Tag with swinging animation */}
        <div
          className="animate-swing relative w-full text-white shadow-[0_2px_4px_rgba(0,0,0,0.15),0_4px_8px_rgba(249,47,123,0.2)] sm:shadow-[0_4px_8px_rgba(0,0,0,0.15),0_8px_16px_rgba(249,47,123,0.2),0_16px_32px_rgba(0,0,0,0.1),0_2px_4px_rgba(0,0,0,0.3)] motion-reduce:animate-none"
          style={{
        ...
        >
          {/* Hole punch at top */}
          <div
            className="absolute left-1/2 top-3 h-6 w-6 sm:top-4 sm:h-8 sm:w-8 -translate-x-1/2 rounded-full bg-gradient-to-br from-gray-50 to-gray-100"
            style={{
          ...
          {/* Model Section */}
          <div className="border-b-2 border-white/90 px-4 py-3 sm:px-6 sm:py-4">
            <div className="mb-1 text-xs font-extrabold uppercase tracking-wider text-black">
              Model
            </div>
            <div className="text-center text-lg sm:text-xl font-black text-white">{data.model}</div>
          </div>
          ...
          <div className="border-b-2 border-white/90 px-4 py-3 sm:px-6 sm:py-4">
            ...
            <div className="text-xl sm:text-2xl font-black text-yellow-400">
              {rangeValue || 'N/A'}
              {rangeValue ? <span className="ml-1 text-sm sm:text-base font-extrabold">miles</span> : null}
            </div>
          </div>
          ...
          <div className="border-b-2 border-white/90 px-4 py-3 sm:px-6 sm:py-4">
            ...
            <div className="text-xl sm:text-2xl font-black text-yellow-400">
              {speedValue || 'N/A'}
              {speedValue ? <span className="ml-1 text-sm sm:text-base font-extrabold">mph</span> : null}
            </div>
          </div>
          ...
          <div className="border-b-2 border-white/90 px-4 py-3 sm:px-6 sm:py-4">
            ...
            <div className="text-xl sm:text-2xl font-black text-yellow-400">{data.weight || 'N/A'}</div>
          </div>

          {/* Notice Section */}
          <div
            className="relative border-white/90 px-3 py-2 sm:px-6 sm:py-3"
            style={{
              backgroundImage:
          ...
          >
            <div className="relative z-10 text-[10px] sm:text-xs font-semibold leading-tight text-black">
          ...
          {/* Price Section */}
          <div className="mt-px border-b-2 border-white/90 px-4 py-4 sm:px-6 sm:py-5">
            <div className="text-center text-2xl sm:text-3xl font-extrabold tracking-tight text-white">
              {formattedPrice}.
            </div>
          </div>
          ...
          <div className="flex items-center justify-center p-4 sm:p-6">
            <Link
              className="w-full transform rounded-lg bg-white px-3 py-2.5 sm:px-4 sm:py-3 text-center text-sm sm:text-base font-bold text-[#F92F7B] shadow-lg transition-all duration-200 hover:-translate-y-0.5 hover:bg-gray-50 hover:shadow-xl"

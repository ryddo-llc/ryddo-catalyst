...
export default function PartnersContactBar() {
  ...
  return (
    <section className="sticky bottom-0 left-0 right-0 z-50 flex w-full flex-col items-stretch bg-black text-xs font-bold text-white sm:text-sm lg:flex-row">
      ...
      <button 
        className="xl:px-18 hidden h-12 items-center justify-center border-b border-white px-3 transition-colors duration-200 hover:bg-[#F92F7B] sm:flex md:h-16 md:border-b-0 md:border-r md:px-10 lg:px-12"
        onClick={() => openPopup('adventures')}
      >
        <div className="h-auto whitespace-nowrap p-0 text-center text-white">
          <span className="hidden md:inline">Sign up for Free ryddo adventures</span>
          <span className="sm:inline md:hidden">Free adventures</span>
          <span className="ml-1 text-[#F92F7B]">^</span>
        </div>
      </button>

      {/* Partners/Brands Section - Hidden on mobile */}
      <div className="hidden min-h-[48px] flex-1 items-center justify-center gap-3 px-3 py-2 sm:flex md:min-h-[64px] md:gap-8 md:px-8 md:py-0 lg:gap-12 lg:px-10 xl:gap-20 2xl:gap-32">
      ...
      <div className="hidden w-full sm:flex lg:w-auto">
        {/* Phone Number */}

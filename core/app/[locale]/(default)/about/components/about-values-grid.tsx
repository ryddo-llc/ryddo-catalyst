      ...
      <div className="grid grid-cols-1 md:grid-cols-1 lg:grid-cols-3 gap-10 md:gap-12">
        {values.map((item) => (
          <div className="flex flex-col items-start relative" key={item.title}>
            ...
            <h3 className="text-2xl font-extrabold text-black font-['Nunito'] w-full relative z-10">
              {item.title}
            </h3>
            <div className="text-base text-gray-700 font-['Nunito'] relative z-10">{item.description}</div>
          </div>
        ))}
      </div>
    </SectionLayout>

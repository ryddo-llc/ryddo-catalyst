  ...
  switch (iconType) {
    ...
    case 'wishlists':
      return {
        ...
        preview:
          dashboardData.wishlistsSummary.totalItems > 0
    ...
    <div className="flex h-full flex-col">
      ...
      <nav className="flex-1 space-y-1 p-4">
        {links.map((link) => {
          ...
          const isActive =
            pathname === link.href ||
            (link.href !== '/logout' && link.href !== '/account' && pathname.startsWith(link.href));
          ...
          return (
            <Link
              className={clsx(
                ...
                isActive
                  ? 'border-r-2 border-[#F92F7B] bg-[#F92F7B]/10 text-[#F92F7B]'
                ...
                isLogout &&
                  'mt-4 border-t border-[var(--account-card-border,hsl(var(--contrast-200)))] pt-4 hover:bg-red-50 hover:text-red-600',
              )}
              href={link.href}
              key={link.href}
        ...
        <div className="border-t border-[var(--account-card-border,hsl(var(--contrast-200)))] p-4">
          <div className="flex items-center gap-3 rounded-lg bg-[var(--account-card-hover,hsl(var(--contrast-50)))] p-3">
            <div className="flex h-8 w-8 items-center justify-center rounded-full bg-[#F92F7B] text-white">
              <span className="text-sm font-bold">{dashboardData.customerInfo.firstName[0]}</span>
            </div>
            <div className="min-w-0 flex-1">
              <div className="truncate text-sm font-medium text-[var(--account-card-title,hsl(var(--foreground)))]">
                {dashboardData.customerInfo.firstName} {dashboardData.customerInfo.lastName}
              </div>
              <div className="truncate text-xs text-[var(--account-card-description,hsl(var(--contrast-500)))]">
